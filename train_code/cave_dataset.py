...
class TrainDataset(Dataset):
    def __init__(self, data_root, crop_size, arg=True, bgr2rgb=True, stride=8):
        ...
        with open(f'{data_root}/split_txt/train_list.txt', 'r') as fin:
            ...
            bgr_list = [line.replace('ms.mat', 'RGB.bmp') for line in hyper_list]
        hyper_list.sort()
        bgr_list.sort()
        print(f'len(hyper) of cave dataset:{len(hyper_list)}')
        print(f'len(bgr) of cave dataset:{len(bgr_list)}')
...
class ValidDataset(Dataset):
    def __init__(self, data_root, bgr2rgb=True):
        ...
        hyper_list.sort()
        bgr_list.sort()
        print(f'len(hyper_valid) of cave dataset:{len(hyper_list)}')
        print(f'len(bgr_valid) of cave dataset:{len(bgr_list)}')
        for i in range(len(hyper_list)):
            hyper_path = hyper_data_path + hyper_list[i]
            if 'mat' not in hyper_path:
                continue
            with h5py.File(hyper_path, 'r') as mat:
                hyper = np.float32(np.array(mat['hsi']))
            hyper = hyper/65536+0.0000000001
            hyper = np.transpose(hyper, [0, 2, 1])
            bgr_path = bgr_data_path + bgr_list[i]
            bgr = cv2.imread(bgr_path)
            if bgr2rgb:
    ...
    def __getitem__(self, idx):
        img_idx, patch_idx = idx//16, idx%16
        h_idx, w_idx = patch_idx//4, patch_idx%4
