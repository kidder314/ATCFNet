...
import os
from ntire_dataset import TrainDataset, ValidDataset
...
parser.add_argument("--outf", type=str, default='./exp/proposed/', help='path log files')
parser.add_argument("--data_root", type=str, default='../dataset_ntire/')
...
def main():
    # load dataset
    print("\nloading dataset ...")
    train_data = TrainDataset(data_root=opt.data_root, crop_size=opt.patch_size, arg=True,
                              stride=opt.stride)
    ...
    while iteration<total_iteration:
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            if iteration % 1000 == 0:
                ...
                mrae_loss, rmse_loss, psnr_loss, sam_loss = validate(val_loader, model, criterion_mrae, criterion_rmse, criterion_psnr, criterion_sam)
                print(f'MRAE:{mrae_loss}, RMSE: {rmse_loss}, PNSR:{psnr_loss}, SAM:{sam_loss}')
                # Save model
                if torch.abs(mrae_loss - record_mrae_loss) < 0.01 or mrae_loss < record_mrae_loss or iteration % 1000 == 0:
...
# Validate
def validate(val_loader, model, criterion_mrae, criterion_rmse, criterion_psnr, criterion_sam):
    model.eval()
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            output = output+0.0000000001
            # output = output.cpu().numpy()
            # target = target.cpu().numpy()
            # Metric = iv.spectra_metric(output, target)
            output1 = output[:, :, 128:-128, 128:-128].cpu().numpy()
            target1 = target[:, :, 128:-128, 128:-128].cpu().numpy()
            Metric = iv.spectra_metric(output1, target1)
            loss_sam = Metric.SAM()
...
class TrainLoss(nn.Module):
    def __init__(self):
        super().__init__()
        self.loss1 = Loss_MRAE()
        self.loss2 = nn.L1Loss()

    def forward(self, y, lab):
        return 0.01*self.loss1(y, lab) + self.loss2(y, lab)
